...
export function PhoneInput({ dialCode, phoneNumber, onDialCodeChange, onPhoneNumberChange, error }: PhoneInputProps) {
  const [open, setOpen] = useState(false)
  const [search, setSearch] = useState("")
  const dropdownRef = useRef<HTMLDivElement>(null)
  const searchRef = useRef<HTMLInputElement>(null)

  const selected = COUNTRIES.find(c => c.dialCode === dialCode) ?? COUNTRIES.find(c => c.code === "NG")!
  ...
  return (
    <div className="flex flex-col gap-1 w-full">
      ...
      <div className={`flex rounded-md border bg-white overflow-visible focus-within:ring-2 focus-within:ring-primary transition-colors ${error ? "border-red-500 focus-within:ring-red-500" : "border-gray-300"}`}>
        {/* Country code selector */}
        <div ref={dropdownRef} className="relative flex-shrink-0">
          <button
            type="button"
            onClick={() => setOpen(v => !v)}
            className="flex items-center gap-1.5 px-3 h-full border-r border-gray-200 text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors rounded-l-md whitespace-nowrap"
          >
                ...
                {filtered.length === 0 ? (
                  ...
                  filtered.map(c => (
                    <li key={c.code}>
                      <button
                        type="button"
                        onClick={() => { onDialCodeChange(c.dialCode); setOpen(false) }}
                        className={`w-full flex items-center gap-3 px-4 py-2.5 text-sm hover:bg-primary/5 transition-colors text-left ${c.dialCode === dialCode ? "bg-primary/10 font-medium text-primary" : "text-gray-700"}`}
                      >
        ...
        <input
          ...
          className="flex-1 px-3 py-2 text-sm outline-none bg-transparent placeholder:text-gray-400 rounded-r-md"
        />
      </div>
